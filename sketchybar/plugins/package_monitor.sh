...
# icon to be displayed if no packages are outdated. Change to `ZERO=""` if you want the widget to be invisible when no packages are out of date. Default: ??
ZERO=""
...
if [[ $SUM -gt 0 ]]; then
  ...
  sketchybar --set packages drawing=off
fi
